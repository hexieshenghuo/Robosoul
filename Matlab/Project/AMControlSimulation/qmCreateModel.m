%% 得到多旋翼3D模型
function [ MultiRotor] = qmCreateModel()

%% ------参数
BodyLength=360;
BodyHeight=20; %也是width 长宽一致
...
w=30;
h=0.06;
ArmBasePos=[0;0; -h/2*1000; 0];
ArmBase=GetBox(l,w,h*1000,ArmBasePos);
...
% 基座
color='k';
MultiRotor=AddComponent(MultiRotor,ArmBase,'Box',color);

%% 把单位从mm变为m

MultiRotor=TransComponent(eye(4)*0.001,MultiRotor);
