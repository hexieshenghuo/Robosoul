%m571a.m    线性离散系统的PID控制（用GA整定PID参数） 
%          仿真系统：y(k)=1.368y(k-1)-0.368y(k-2)+0.368u(k-1)+0.264u(k-2)              
%          二进编码
%          适应度函数=1/总目标函数

%%%%%%%    只画最优Kp、Ki、Kd参数的PID控制系统的输出  
clear all;
close all;
...
Ki
Kd
figure(2)   ;%画每代总目标函数最小值Jmin、平均值Jmean                   
t=1:T;               
...
ee=[0;0;0];   
for k=2:100
      u(k)=Kp*ee(1)+Ki*ee(2)+Kd*ee(3);%PID控制器输出
      y(k)=1.368*y1-0.368*y2+0.368*u1+0.264*u2   ;%仿真系统输出
      e(k)=r(k)-y(k);     
     y1=y(k);y2=y1;
     u1=u(k);u2=u1;
 ...
 figure(3)  ;%画Kp、Ki、Kd参数的PID控制系统的输出
 plot(y,'r');hold on;plot(r,'b'); xlabel('k');ylabel('y(k)'),pause